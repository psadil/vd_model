...
for layer=1:p.layer
    ...
    for grid = 1:p.nGrids(layer),
        ...
        for cycle=1:p.numEncodingCycles
            ...
            if forTrain
                % Update Weights
                weights = (weights + f.*(input_mat-repmat(dot(weights,weights,3),[1,1,p.numInputDims(layer)]).*input_mat)) ./ ...
                    repmat(squeeze(sqrt(sum((weights + f.*(input_mat-repmat(dot(weights,weights,3),[1,1,p.numInputDims(layer)]).*input_mat)).^2,3))),...
