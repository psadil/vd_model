...
for sess = 2:size(RUN_SIM, 1),
    ...
    if p.stimCond == 3
        p.fixn_ratio = p.fixn_ratio_lowHigh(p.stimCond-2);
    else
    ...
    % 'match' will be made)
    if p.stimCond == 3
        p.maxFix = p.maxFixations(p.stimCond-2);
    else
    ...
    if p.stimCond == 3
        p.nEncodCycles = p.numEncodingCycles(p.stimCond-2);
    else
