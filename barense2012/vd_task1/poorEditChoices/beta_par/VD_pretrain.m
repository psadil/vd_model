function [p,w_full] = VD_pretrain(p,rat)
...
w_full = rand(2,p.numRows,p.numRows,p.numInputDims(2),p.nGrids(1));
...
for layer = 1:max(p.numLayers)
    ...
    for grid = 1:p.nGrids(layer),
        ...
        w = squeeze(squeeze(w_full(layer,:,:, ...
            firstFeatureToCheck(grid):lastFeatureToCheck(grid),grid)));
        ...
        for cycle=1:p.numTrainCycles(layer),
            ...
            w_full(layer,:,:,...
                firstFeatureToCheck(grid):lastFeatureToCheck(grid),grid) = w;
