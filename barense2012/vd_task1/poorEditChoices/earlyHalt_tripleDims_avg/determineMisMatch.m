...
thresh = mean(p.famil_diff_thresh(1,:));

lower_caudal_thresh = thresh - noise;
upper_caudal_thresh = thresh + noise;

thresh = lower_caudal_thresh + (upper_caudal_thresh - lower_caudal_thresh)*rand;
...
if p.layer == 2
    ...
    familDiff = mean([familDiff_caudal, familDiff_PRC]);
    p.usePRC(:,trial) = 1;
